...
static uint32_t port_create(uint32_t index, coap_port_t  * p_port)
{
    ...
    return (err_code == kThreadError_None) ? NRF_SUCCESS : NRF_ERROR_INTERNAL;
}
                              ...
                              uint16_t               datalen)
{
    ...
    do {
		...
		if (otMessageSetLength(p_msg, datalen) != kThreadError_None)
		{
		...
		COAP_MUTEX_UNLOCK();
		if (otUdpSend(p_socket, p_msg, &msg_info) != kThreadError_None)
		{
