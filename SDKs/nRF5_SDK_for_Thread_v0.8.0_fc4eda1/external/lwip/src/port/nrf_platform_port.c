...
static __INLINE bool eui64_compare(const eui64_t * a, const eui64_t * b)
{
    return (memcmp(a->identifier, b->identifier, EUI_64_ADDR_SIZE) ? false : true);
}

static void blenetif_6lowpan_interface_search_and_update(iot_interface_t     * p_interface,
                                                         struct blenetif    ** p_blenetif)
{
    uint32_t index;

    for (index = 0; index < BLE_6LOWPAN_MAX_INTERFACE; index++)
    {
        // Search criteria for matching iot interface is that local and peer EUIs are identifical.
        // The pointer reference to the IoT interface is updated each time there is a match.
        if (eui64_compare(&p_interface->local_addr, &m_blenetif_table[index].p_ble_interface->local_addr) &&
            eui64_compare(&p_interface->peer_addr, &m_blenetif_table[index].p_ble_interface->peer_addr))
        {
            (*p_blenetif) = &m_blenetif_table[index];
            m_blenetif_table[index].p_ble_interface = p_interface;
            break;
        }
    }
}
                                        ...
                                        ble_6lowpan_event_t * p_event)
{
   ...
   blenetif_6lowpan_interface_search_and_update(p_interface, &p_blenetif);

    switch(p_event->event_id)
    {
        case BLE_6LO_EVT_INTERFACE_ADD:
        {
            // If no exisiting interface was found, allocate a new one.
            if (p_blenetif == NULL)
            {
                for (index = 0; index < BLE_6LOWPAN_MAX_INTERFACE; index++)
                {
            ...
            {
                // Interface already exists, notify lwIP that the link is up.
                netif_set_link_up(&p_blenetif->netif);
                break;
            }
        ...
        case BLE_6LO_EVT_INTERFACE_DELETE:
        {
            if (p_blenetif != NULL)
            {
                netif_set_link_down(&p_blenetif->netif);
